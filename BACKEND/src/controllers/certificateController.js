...
export const verifyCertificate = async (req, res) => {
  try {
    ...
    // 🔹 Require certificateId
    if (!certificateId || typeof certificateId !== "string") {
      return res.status(400).json({
        success: false,
        error: "CertificateId is required and must be a string",
      });
    }
    ...
    // 🔹 Fetch student info linked to certificate
    const result = await pool.query(
      `SELECT spi.student_id, spi.aadhar_number, spi.email, spi.pan_number, spi.phone,
       ...
       WHERE suq.certificate_id = $1 
         AND suq.certificate_status = 'completed'`,
      [certificateId.trim()]
    );
    ...
    if (email && email.toLowerCase() === student.email.toLowerCase()) matches++; // case-insensitive
    if (pan && pan.toUpperCase() === student.pan_number.toUpperCase()) matches++; // normalize PAN
    if (phone && phone === student.phone) matches++;

    if (matches !== 2) {
      return res.status(401).json({
        success: false,
        error: "Verification failed: provided fields do not match records",
      });
    }

    // ✅ Success → return certificate details
    return res.json({
      success: true,
